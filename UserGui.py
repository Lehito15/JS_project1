...
class UserGui(QtCore.QObject):
    ...
    def load_game(self):
        ...
        if count == 0:
            ...
            for league in leagues:
                ...
                item.setText(f'Twój zespół {t.cluv_by_code(league[0])} ostatnia kolejka {league[1]}.')
    ...
    def load_db(self, info, user):
        ...
        print(type(my_team))
        del info[0]
        team_dict = {team[1]: Team.Team(team[0], team[1], team[2], team[3], team[4], team[5]) for team in info}
        list_team = list(team_dict.values())
        self.team_guii.league = League.League(list_team, user, nr_round, team_dict[my_team])
        self.team_guii.league.team_dict_objects = team_dict
        self.team_guii.league.my_team = team_dict[my_team]
        self.start.hide()
        self.team_guii.show()
